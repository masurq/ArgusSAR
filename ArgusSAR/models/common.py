...
class MHABlock(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads):
        ...
        self.W_q = nn.ModuleList([nn.Linear(embed_dim, self.d_k) for _ in range(num_heads)])
        self.W_k = nn.ModuleList([nn.Linear(embed_dim, self.d_k) for _ in range(num_heads)])
        self.W_v = nn.ModuleList([nn.Linear(embed_dim, self.d_k) for _ in range(num_heads)])
        self.W_o = nn.Linear(num_heads * self.d_k, embed_dim)

    def forward(self, x):
        # YOLOv5 输入的 x 为 [B, C, H, W]
        B, C, H, W = x.shape
        # 将特征图展平为序列格式 [B, n, d] (n=H*W, d=C)
        E = x.view(B, C, H * W).permute(0, 2, 1)
        ...
        for i in range(self.num_heads):
            ...
            scores = torch.matmul(Q_i, K_i_T) / math.sqrt(self.d_k)
            A_i = torch.softmax(scores, dim=-1)
            Z_i = torch.matmul(A_i, V_i)
            Z_heads.append(Z_i)

        Z_merged = torch.cat(Z_heads, dim=-1)
        Weighted_Emb = self.W_o(Z_merged)

        out_features = Weighted_Emb.permute(0, 2, 1).contiguous().view(B, C, H, W)
        return out_features



class KernelPredictionModule(nn.Module):
    def __init__(self, in_channels, cm, sigma, k_encoder, k_up):
        super(KernelPredictionModule, self).__init__()
        self.sigma = sigma
        self.k_up = k_up
        self.conv1x1 = nn.Conv2d(in_channels, cm, kernel_size=1)
        padding = k_encoder // 2
        out_channels_map = (sigma ** 2) * (k_up ** 2)
        self.encoder_conv = nn.Conv2d(cm, out_channels_map, kernel_size=k_encoder, padding=padding)
        self.pixel_shuffle = nn.PixelShuffle(sigma)

    def forward(self, x):
        x_prime = self.conv1x1(x)
        k_flat = self.encoder_conv(x_prime)
        k_pred = self.pixel_shuffle(k_flat)
        k_norm = F.softmax(k_pred, dim=1)
        return k_norm


class ContentAwareReconstructionModule(nn.Module):
    def __init__(self, sigma, k_up):
        super(ContentAwareReconstructionModule, self).__init__()
        self.sigma = sigma
        self.k_up = k_up

    def forward(self, x, k_norm):
        B, C, H, W = x.shape
        pad = self.k_up // 2
        x_unfolded = F.unfold(x, kernel_size=self.k_up, padding=pad)
        x_unfolded = x_unfolded.view(B, C, self.k_up ** 2, H, W)
        feat_up = x_unfolded.repeat_interleave(self.sigma, dim=-2).repeat_interleave(self.sigma, dim=-1)
        w = k_norm.unsqueeze(1)
        y = (feat_up * w).sum(dim=2)
        return y


class FFAEContentAwareUpsampling(nn.Module):

    def __init__(self, in_channels, sigma, cm, k_encoder=3, k_up=5):
        super(FFAEContentAwareUpsampling, self).__init__()
        self.kernel_predictor = KernelPredictionModule(in_channels, cm, sigma, k_encoder, k_up)
        self.reconstructor = ContentAwareReconstructionModule(sigma, k_up)

    def forward(self, x):
        k_norm = self.kernel_predictor(x)
        out = self.reconstructor(x, k_norm)
        return out


class MAADPerceptionLayer(nn.Module):
    def __init__(self, in_channels, reduction=4):
        super(MAADPerceptionLayer, self).__init__()
        self.in_channels = in_channels


        self.c_hidden = max(in_channels // 4, 1)
        self.input_conv = nn.Sequential(
            nn.Conv2d(in_channels, self.c_hidden, kernel_size=1, bias=False),
        ...
        concat_channels = self.c_hidden * 4

        self.out_conv = nn.Conv2d(concat_channels, in_channels, 1,
                                  1) if concat_channels != in_channels else nn.Identity()

        self.pi_L_attention = nn.Sequential(
        ...
        self.pi_S_conv = nn.Conv2d(
            in_channels, 3 * (self.deform_kernel_size ** 2), kernel_size=3, padding=1
        )
        nn.init.constant_(self.pi_S_conv.weight, 0)
        nn.init.constant_(self.pi_S_conv.bias, 0)
        self.pi_S_weight = nn.Parameter(
            torch.empty(in_channels, in_channels, self.deform_kernel_size, self.deform_kernel_size)
        )
        nn.init.kaiming_uniform_(self.pi_S_weight, a=math.sqrt(5))

        hidden_dim = max(in_channels // reduction, 16)
        self.pi_C_hypernet = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Flatten(),
            nn.Linear(in_channels, hidden_dim),
            nn.SiLU(inplace=True),
            nn.Linear(hidden_dim, 4 * in_channels),
    ...
    def forward(self, x):
        ...
        f_conv = self.input_conv(x)
        p1 = self.pool(f_conv)
        p2 = self.pool(p1)
        p3 = self.pool(p2)
        f_pooled = self.out_conv(torch.cat([f_conv, p1, p2, p3], dim=1))

        A_L = self.pi_L_attention(f_pooled)
        f_L = f_pooled * A_L

        f_L = f_L.contiguous()
        offset_mask = self.pi_S_conv(f_L)
        delta_p = offset_mask[:, :18, :, :].contiguous()
        delta_m = torch.sigmoid(offset_mask[:, 18:, :, :]).contiguous()

        if f_L.device.type == 'cpu':
            f_S_deform = F.conv2d(f_L, self.deform_weight, padding=1)
        else:
            f_S_deform = torchvision.ops.deform_conv2d(
                input=f_L, offset=delta_p, weight=self.deform_weight, mask=delta_m, padding=1
        )
        f_S = f_L + f_S_deform

        theta = self.pi_C_hypernet(f_S).view(B, 4, -1, 1, 1)
        alpha1, beta1, alpha2, beta2 = theta[:, 0], theta[:, 1], theta[:, 2], theta[:, 3]

        branch1 = alpha1 * f_S + beta1
        branch2 = alpha2 * f_S + beta2
        f_out = torch.max(branch1, branch2)

        return f_out

    @property
    def deform_weight(self):
        return self.pi_S_weight


class MAADDecoder(nn.Module):

    def __init__(self, in_channels, num_layers=2):
        super(MAADDecoder, self).__init__()
        self.layers = nn.Sequential(*[
            MAADPerceptionLayer(in_channels=in_channels) for _ in range(num_layers)
        ])
