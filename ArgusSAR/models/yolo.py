    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m in {BottleneckCSP, C3, C3TR, C3Ghost, C3x}:
                ...
                n = 1
        elif m in {MHABlock, FFAEContentAwareUpsampling, MAADDecoder}:
            c1, c2 = ch[f], ch[f]  # 输入通道数 c1，输出通道数 c2（保持不变）
            args = [c1, *args]  # 将输入通道数自动作为第一个参数插入，后面的参数从 yaml 中读取
